  ...
  UI.screens.profile = {
    ...
    render() {
      ...
      return (
        ...
        '</div>' + (s.lg ? '<p class="muted small">في الدوري وحده: ' + s.lg.ap + ' مباراة · ' + s.lg.g + ' هدف · ' + s.lg.a + ' صناعة (الأرقام أعلاه لكل مسابقات النادي)</p>' : '') +
        '<h4>المسيرة مع الأندية</h4><p>' + c.ap + ' مباراة · ' + c.g + ' هدف · ' + c.a + ' صناعة · ' + c.motm + ' مرة رجل المباراة</p>' +
        (function () {
          const I = u.intl;
          if (!I) return '';
          return '<h4>' + UI.flag(u.nat, 12) + ' المنتخب</h4><p>' + (I.caps ? I.caps + ' مباراة دولية · ' + I.g + ' هدف · ' + I.a + ' صناعة' + (I.capt ? ' · القائد 🅒' : '') : 'لم تلعب دولياً بعد') + ' <button class="link" data-go="nation">صفحة المنتخب</button></p>';
        })() +
        '</div>' +
        '</div>' +
        (function () {
        ...
        (function () {
          const h = u.clubHist || [];
          if (!h.length) return '';
          const T = { academy: 'الأكاديمية', transfer: 'انتقال', loan: 'إعارة', free: 'انتقال حر' };
          return '<div class="panel"><h3>مسيرتك بالأندية</h3><div class="fixtures">' + h.slice().reverse().map((x) => {
            const c = st.clubs[x.club];
            return '<div class="fx-row"><span class="fx-d">' + FC.Calendar.seasonLabel(x.s) + '</span><span class="fx-o">' + (c ? UI.badge(c, 18) + ' ' + esc(c.name) : '—') + '</span><span class="muted small">' + (T[x.type] || '') + (x.fee ? ' · ' + esc(FC.Econ.fmt(x.fee)) : '') + '</span></div>';
          }).join('') + '</div><div class="kv"><span>القيمة السوقية الآن</span><b dir="ltr">' + esc(FC.Econ.fmt(FC.Econ.value(st, u))) + '</b></div></div>';
        })() +
